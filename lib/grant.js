...
function Grant (config, req, res, next) {
  ...
  runner(this.fns, this, next);
}

extend(Grant, {
  _hook: function (after, fnName, hookName, hookFn) {
    var insLoc = Grant.prototype.fns.indexOf(fnName);
    if(insLoc != -1) {
      if(fnName != hookName) {
        Grant.prototype.fns.splice(insLoc + (after ? 1 : 0), 0, hookName);
      }
      Grant.prototype[hookName] = hookFn;
      return true;
    }
    return false;
  },

  hookBefore: function hookBefore (fnName, hookName, hookFn) {
    return Grant._hook(false, fnName, hookName, hookFn);
  },

  hookAfter: function hookAfter (fnName, hookName, hookFn) {
    return Grant._hook(true, fnName, hookName, hookFn);
  }
});

extend(Grant.prototype, {
  /**
   ...
   * @this   OAuth
   */
  extractCredentials: function extractCredentials (done) {
    ...
    this.client = this.credsFromBasic(this.req) || this.credsFromBody(this.req);
   ...
   */
  credsFromBasic: function credsFromBasic (req) {
    var user = auth(req);
   ...
   * @return {Object} Client
   */
  credsFromBody: function credsFromBody (req) {
   ...
   * @this   OAuth
   */
  checkClient: function checkClient (done) {
   ...
   * @this   OAuth
   */
  checkGrantType: function checkGrantType (done) {
    if (this.grantType.match(/^http(s|):\/\//) && this.model.extendedGrant) {
      return this.useExtendedGrant.call(this, done);
    }

    switch (this.grantType) {
      case 'authorization_code':
        return this.useAuthCodeGrant.call(this, done);
      case 'password':
        return this.usePasswordGrant.call(this, done);
      case 'refresh_token':
        return this.useRefreshTokenGrant.call(this, done);
      case 'client_credentials':
        return this.useClientCredentialsGrant.call(this, done);
      default:
   ...
   * @param  {Function} done
   */
  useAuthCodeGrant: function useAuthCodeGrant (done) {
   ...
   * @param  {Function} done
   */
  usePasswordGrant: function usePasswordGrant (done) {
    // User credentials
   ...
   * @param  {Function} done
   */
  useRefreshTokenGrant: function useRefreshTokenGrant (done) {
   ...
   * @param  {Function} done
   */
  useClientCredentialsGrant: function useClientCredentialsGrant (done) {
    // Client credentials
   ...
   * @param  {Function} done
   */
  useExtendedGrant: function useExtendedGrant (done) {
    var self = this;
   ...
   * @this   OAuth
   */
  checkGrantTypeAllowed: function checkGrantTypeAllowed (done) {
   ...
   * @this   OAuth
   */
  generateAccessToken: function generateAccessToken (done) {
    var self = this;
   ...
   * @this   OAuth
   */
  saveAccessToken: function saveAccessToken (done) {
    ...
    if (this.config.accessTokenLifetime !== null) {
      expires = new Date(this.now.getTime() + (this.config.accessTokenLifetime * 1000));
    }
   ...
   * @this   OAuth
   */
  generateRefreshToken: function generateRefreshToken (done) {
   ...
   * @this   OAuth
   */
  saveRefreshToken: function saveRefreshToken (done) {
  ...
  /**
   * Prepare a response to send to client
   *
   * @param  {Function} done
   * @this   OAuth
   */
  prepareResponse: function prepareResponse (done) {
    var response = {
      token_type: 'bearer',
   ...
   * @this   OAuth
   */
  sendResponse: function sendResponse (done) {
    this.res.jsonp(this._response);
